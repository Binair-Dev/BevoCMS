package be.bnair.bevo.repository;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import be.bnair.bevo.models.entities.ShopCategoryEntity;

/**
 * Interface de repository pour la gestion des entités de catégories de boutique (ShopCategoryEntity).
 * Cette interface permet d'effectuer des opérations CRUD (Create, Read, Update, Delete)
 * sur les entités de catégories de boutique en utilisant Spring Data JPA.
 *
 * © 2023 Brian Van Bellinghen. Tous droits réservés.
 */
@Repository
public interface ShopCategoryRepository extends JpaRepository<ShopCategoryEntity, Long> {
}